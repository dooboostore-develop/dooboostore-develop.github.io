...
class Server implements Runnable<void, void> {
  async run() {
    ...
    const repositoryResourceFilter = new ResourceFilter(PathUtils.resolve(__dirname,'../../../packages'),[
      {
        request: (rr) => {
          return rr.reqUrlPathName.startsWith("/dooboostore-develop.github.io/packages");
        },
        dist: (rr) => {
    ...
    const option = new HttpSSRServerOption(
      {
        ...
        filters: [repositoryResourceFilter, resourceFilter, ssrFilter, IntentSchemeFilter],
      },
      {
