...
export class SimpleBootHttpServerSsrRouteComponent extends ComponentBase {
  ...
  async onInitRender(param: any, rawSet: RawSet): Promise<void> {
    ...
    if (ValidUtils.isBrowser() && this.exampleProject) {
      ...
      if (StackBlitzSDK) {
        StackBlitzSDK.embedProjectId(this.exampleProject, 'stackblitz-starters-k8onfnj3', {
          ...
          terminalHeight: 80  // 터미널 높이 설정 (퍼센트, 0~100). 30%로 터미널 열기
        }).then((vm:any) => {
